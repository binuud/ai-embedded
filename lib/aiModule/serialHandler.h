...
// this is prefix match, commands coming first, will take precendence in prefix
// match
...
public:
  ...
  void help() {
    ...
    Serial.println("  All set commands will have the AT command followed by = "
                   "symbol and values");
    Serial.println("  All read commands will have just the AT command without "
                   "any further symbols or values");
  ...
  void loop() {
    ...
    if (Serial.available() > 0) {
      ...
      for (int i = 0; i < MAX_COMMANDS; i++) {
        ...
        // do not capitalize commands here, since 
        // params too will get capitalized
        if (cmdLine.startsWith(cmdName)) {
          String params = input.substring(
              3 + cmdName.length()); // to prevent auto uppercase, done above to
